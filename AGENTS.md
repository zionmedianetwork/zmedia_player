...
test/                             # Dart tests (core, models, services, widgets, memory, performance, exceptions, security, crash_reporting) — run `flutter test` for the current count
example/                         # Feature-per-page gallery app (verified on a physical iPhone); has its own test/ suite too
...
- Run `flutter analyze` (clean) and `flutter test` (keep green — run it for the current count) before proposing changes; if you touched `example/`, also run `cd example && flutter analyze` (clean) and `cd example && flutter test` (24 tests as of this writing) — CI runs both on every push and pull request via the `example-tests` job.

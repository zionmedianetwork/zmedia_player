...
| **Error Handling** | `pages/error_handling_page.dart` | `MediaPlayerException` hierarchy · `PlayerState.error` · `PlaybackState.errorMessage` · `player.stateStream` · `player.errorStream` · `player.pauseReasonStream`. Six scenarios: valid URL, 403, 404, bad host, unsupported format, and a **Load Watchdog** scenario for `MediaConfig.loadTimeout` (issue #125, lowered to 8s from the 30s default — **requires Airplane Mode**, since no reachable URL can produce "accepted then total silence"). The Live Player State card is the on-device readout for #125's terminal-`error` guarantee: after any failure it must read `error` and stay there, never settling back on `paused`/`idle`. `pauseReasonStream` entries are logged alongside errors as pause *attribution* (issue #126) — tap pause → `user`, unplug headphones → `audioBecomingNoisy` (Android only), take a call/trigger Siri → `audioFocusLoss` |
...
| **Wired Config Verification** | `pages/wired_config_verification_page.dart` | On-device harness for `HlsConfig.enableDvr`/`.liveLatency`, `NotificationConfig.customActions`/`.priority`/`.dismissible` (Android only), and `PipConfig.actions` (Android only). Its Live Latency section documents that `liveLatency` is a **join target** on both platforms and a *maintained* one on **iOS only** (`automaticallyPreservesTimeOffsetFromLive = true` restores the cushion after each rebuffer via a visible forward skip, with no opt-out); on Android nothing holds the target after the join, because ExoPlayer's live playback-speed control is switched off for ordinary HLS/DASH streams (issue #110), and on Android/DASH a manifest time-anchor defect can defeat even the join target. Also renders `PlaybackState.liveEdgeOffset`/`.isAtLiveEdge`/`.positionBasis` (issue #88) as their own rows plus a color-coded case banner — the only on-device way to verify the Android `getCurrentLiveOffset()` bounding fix (issue #109) and see the issue #110 tell, since CI never builds native code and every test mocks the channel. The banner and its disclaimer also document that `liveEdgeOffset` measures a different quantity on each platform and the values are not comparable (issue #120) — on iOS the field is bounded near zero by construction during live playback, so the banner reads "at the live edge" almost continuously there, unlike Android's 15-30s-typical reading. The Source selector's third **Custom** option points the harness at any stream URL — with `MediaItem.isLive`, an explicit `streamingFormat` override (`auto`/`hls`/`dash`/`progressive`, with the *resolved* format shown so a URL that infers to `progressive` doesn't silently defeat `enableDvr`/`liveLatency`), and one optional HTTP header/value pair for a signed/token-gated origin — so the next misbehaving live stream (e.g. one whose manifest time anchor disagrees with its own segment timeline, like the #109/#110 defect stream) can be verified the same way, without a rebuild |
